    ...
    gebo_sidebar = {
			...
			if($(window).width() > 979){
                if(!$('body').hasClass('sidebar_hidden')) {
                    if( $.cookie('gebo_sidebar') == "hidden") {
                        ...
                        $('.sidebar_switch').toggleClass('on_switch off_switch').attr('title','展开');
                    }
                } else {
                    $('.sidebar_switch').toggleClass('on_switch off_switch').attr('title','展开');
                }
        ...
        update_scroll: function() {
			if($('.sidebar_inner').length) {
				if( $(window).width() > 979 ){
					$('.sidebar_inner').height($(window).height() - 40);
				} else {
					$('.sidebar_inner').height('400px');
				}
				//antiScroll.refresh();
			}
        },
        update_acc_status : function(acc_id, li_id){
			$("#acc_head_" + acc_id).addClass("sdb_h_active");
			$("#acc_body_" + acc_id).addClass("in");
			$("ul li.active").removeClass("active");
			$("#nav_"+ li_id).addClass("active");	
        }
    };
